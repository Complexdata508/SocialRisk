 ...
 * 发送HTTP请求从后台获取数据
 ****/
app.controller("institutionInfoController",function($scope,$http,$log){
    ...
    $scope.updateSelection=function($event,institutionNo){
        ...
        if($event.target.checked){
            //如果选中复选框，则将该id号增加到数组中去
            $scope.selectinstitutionNos.push(institutionNo);
        }else{
            //取消删除，则从数组中移除该机构号
            var idx = $scope.selectinstitutionNos.indexOf(institutionNo);   //获取id号对应的下标
	...
	$scope.getPage=function(page,size){
		if(flag){
			$http({
		        method: 'GET',
		        url: 'getPage.shtml?pageNum='+page+'&size='+size
		        	+"&logno="+$scope.logno+"&institutionNo="+$scope.institutionNo
		    }).then(function successCallback(response) {
		    		//集合数据
	...
	$scope.getAll=function(){
		$http({
	        method: 'GET',
	        url: '/itfin_manager_ui/institutionInfo/getAll.shtml',
